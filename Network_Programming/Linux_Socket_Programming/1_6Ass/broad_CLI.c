/*
 *	a simple UDP-Client Socket application that uses broadcasting operation
 *
 *	@Author	- Yuvraj Takey
...
int main()
{
	...
	if((retval = setsockopt(sock_fd,SOL_SOCKET, SO_BROADCAST,&retval, sizeof(retval))) < 0)
	{
	...
	addr.sin_addr.s_addr = htonl(BROAD_RECV);
