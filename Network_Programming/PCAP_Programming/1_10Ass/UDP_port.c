/*
 *	a program to capture the packets and if the packet is of UDP type then print the source
 	...
 	switch((USHORT)ntohs(st_ethr -> type))
 	{
 		case ETHERNET_IP:
 		{
 			...
 			switch(ntohs(st_ipv4 -> Protocol))
 			{
 				...
 				{
 					// get the UDP packets
 					udp_header *udp = (udp_header *) (bytes + h_len + (st_ipv4 -> ip_hl * 4) );
 					
 					// print the port numbers
 					printf("[PROTO_] : UDP\n");
 					printf("src PORT Number \t: %hu\n",ntohs(udp -> src_port));
 					printf("dst PORT Number \t: %hu\n\n",ntohs(udp -> dst_port));
 					break;
 				}
 			}
